    ...
    var funcMaps by remember(vm, ifSaved, vmUI, navController, hazeState) {
        mutableStateOf(
            listOf(
                ...
                SearchAppBean(2,"${context.getString(AppNavRoute.ScanQrCode.label)} 扫码 指尖工大 CAS统一认证登录", { Scan() }, AppNavRoute.ScanQrCode.route,isHigh = true),
                ...
                SearchAppBean(6,"${context.getString(AppNavRoute.Exam.label)}安排 教务处考试安排 ${context.getString(R.string.navigation_label_exam_news)}" , { Exam() },AppNavRoute.Exam.withArgs()),
                SearchAppBean(7,"${context.getString(AppNavRoute.Grade.label)}", { Grade(ifSaved) }, AppNavRoute.Grade.receiveRoute()),
                SearchAppBean(8,"${context.getString(AppNavRoute.FailRate.label)}", { FailRate() },AppNavRoute.FailRate.route),
                SearchAppBean(9,"${context.getString(AppNavRoute.TermCourses.label)} 教材 课本", { CourseTotal(ifSaved) },AppNavRoute.TermCourses.receiveRoute()),
                SearchAppBean(10,"${context.getString(AppNavRoute.PersonInfo.label)}", { PersonUI() },AppNavRoute.PersonInfo.route),
                SearchAppBean(11,"${context.getString(AppNavRoute.WebFolder.label)} 实验室 ${context.getString(R.string.navigation_label_hui_xin)}", { WebUI() }, AppNavRoute.WebFolder.route),
                ...
                SearchAppBean(13,"${context.getString(AppNavRoute.SelectCourses.label)}", { SelectCourse(ifSaved) },AppNavRoute.SelectCourses.route),
                SearchAppBean(14,"${context.getString(AppNavRoute.Dormitory.label)} 寝室卫生评分 寝室卫生分数", { DormitoryScoreXuanCheng() }, AppNavRoute.Dormitory.route),
                SearchAppBean(15,"${context.getString(AppNavRoute.Notifications.label)} ${context.getString(R.string.navigation_label_notifications)} 收纳", { NotificationsCenter() },AppNavRoute.Notifications.route),
                SearchAppBean(16,"${context.getString(AppNavRoute.Survey.label)} 教师评教 教师教评", { Survey(ifSaved) },AppNavRoute.Survey.route),
                SearchAppBean(17,"${context.getString(AppNavRoute.News.label)} 新闻 教务处", { News() },AppNavRoute.News.route),
                SearchAppBean(18,"${context.getString(AppNavRoute.Program.label)}完成情况", { Program(ifSaved) },AppNavRoute.Program.receiveRoute()),
                SearchAppBean(19,"${context.getString(AppNavRoute.Library.label)} 座位预约 ${context.getString(R.string.navigation_label_library_borrowed)}", { LibraryItem() },AppNavRoute.Library.route),
                SearchAppBean(20,"${context.getString(AppNavRoute.Bus.label)}", { SchoolBus() }, AppNavRoute.Bus.route),
...
//                SearchAppBean(22,"${AppNavRoute.NextCourse.label}", { NextCourse(ifSaved,navController ) },AppNavRoute.NextCourse.receiveRoute()),
                ...
                SearchAppBean(24,"${context.getString(AppNavRoute.Classroom.label)} 空教室", { Classroom() },AppNavRoute.Classroom.route),
                SearchAppBean(25,"体育 云运动 乐跑 校园跑 ${context.getString(R.string.navigation_label_physical_fitness_test)} 体育测试 体检", { LePaoYun() }, AppNavRoute.WebView.shareRoute(MyApplication.PE_HOME_URL)),
                SearchAppBean(26,"${context.getString(AppNavRoute.WorkAndRest.label)} 校历", { WorkAndRest() },AppNavRoute.WorkAndRest.withArgs()),
                SearchAppBean(27,"${context.getString(R.string.navigation_label_chsi)}", { XueXin() },AppNavRoute.WebView.shareRoute(MyApplication.XUE_XIN_URL)),
                SearchAppBean(28,"${context.getString(AppNavRoute.Life.label)} 校园 天气 新生 地图", { Life() },AppNavRoute.Life.withArgs(false)),
                SearchAppBean(29,"${context.getString(AppNavRoute.TransferMajor.label)}", { Transfer(ifSaved) }, AppNavRoute.TransferMajor.route),
                SearchAppBean(30,"${context.getString(AppNavRoute.CourseSearch.label)} 全校开课 课程", { CoursesSearch(ifSaved) }, AppNavRoute.CourseSearch.route),
                SearchAppBean(31,"${context.getString(AppNavRoute.TeacherSearch.label)} 老师检索", { TeacherSearch() }, AppNavRoute.TeacherSearch.route),
                SearchAppBean(32,"${context.getString(AppNavRoute.Fee.label)} 费用 欠缴学费", { Pay() }, AppNavRoute.Fee.route),
//                SearchAppBean(33,"实习", { Practice(ifSaved) }),
                SearchAppBean(34,"${context.getString(AppNavRoute.Alumni.label)} 校友 毕业", { Alumni() },AppNavRoute.Alumni.route),
                SearchAppBean(35,"${context.getString(AppNavRoute.StuTodayCampus.label)} 学工系统 学工平台 请假 助学金 奖学金 贫困 寝室 心理 日常", { ToadyCampus() }, AppNavRoute.StuTodayCampus.route),
                SearchAppBean(36,"${context.getString(R.string.navigation_label_ietp)} 大学生创新创业系统", { IETP() }, AppNavRoute.WebView.shareRoute(MyApplication.IETP_URL)),
                SearchAppBean(37,"${context.getString(AppNavRoute.Work.label)} 实习 春招 双选 秋招", { Work() }, AppNavRoute.Work.route),
                SearchAppBean(38,"${context.getString(AppNavRoute.Holiday.label)} 国家法定节假日 假期 节日 调休", { Holiday() },AppNavRoute.Holiday.route),
                ...
                SearchAppBean(41,"${context.getString(AppNavRoute.Admission.label)} 历年分数线 招生计划", { Admission() }, AppNavRoute.Admission.route),
                SearchAppBean(42,"${context.getString(AppNavRoute.WebVpn.label)} 外地访问 内网", { WebVpn() },AppNavRoute.WebVpn.route),
                SearchAppBean(43,"${context.getString(AppNavRoute.OfficeHall.label)}", { OfficeHall() },AppNavRoute.OfficeHall.route),
                SearchAppBean(44,"${context.getString(R.string.navigation_label_hui_xin)} ${context.getString(R.string.navigation_label_hui_xin)} 校园卡 账单 充值 缴费 合肥" , { HuiXin() }, AppNavRoute.WebView.shareRoute(getHuiXinURL())),
                SearchAppBean(45,"${context.getString(AppNavRoute.SecondClass.label)}", { SecondClass() }, AppNavRoute.SecondClass.route),
                SearchAppBean(46,"${context.getString(AppNavRoute.CommunityAppointment.label)} 场地预约 座位预约 宿舍自习室预约 智慧社区平台", { Appointment() }, AppNavRoute.CommunityAppointment.route),
                SearchAppBean(47,"AI 人工智能 ${context.getString(R.string.navigation_label_ai)}", { AI() }, AppNavRoute.AI.route),
                SearchAppBean(48,"事务跟踪 事务追踪 issue 反馈 开发 ${context.getString(R.string.navigation_label_track)}", { Track() }, AppNavRoute.Track.route),
            )
        )
    }
