...
//        UpdateItems("重构 提纯出CAS统一认证登录逻辑，为更多平台的一键登录做准备", null,UpdateType.RENEW)
        UpdateItems("重构 网络请求层，重新进行封装，使用Flow代替LiveData，优化潜在的内存泄漏问题，增加不同状态下的展示", "进度: 完成26/总99",UpdateType.RENEW)
        UpdateItems("新增 就业信息网的检索", "位于 查询中心", UpdateType.ADD)
        UpdateItems("优化 通知公告显示重复项目", null, UpdateType.ADD)
        UpdateItems("修复 安卓10及以下设备不支持组件模糊而导致某些隐藏功能失效的Bug", null, UpdateType.FIX)
