...
fun openOperation(info : String,context: Context) {
    ...
    else if(info.contains("U校园")) Starter.startWebUrlOuter(context,if(!info.contains("AI",ignoreCase = true)) Constant.UNIPUS_URL else Constant.UNIPUS_AI_URL )
    ...
    else if(isValidWebUrl(info)) Starter.startWebUrlOuter(context,info)
