            ...
            if (records.isEmpty()) {
                CustomCard(color = cardNormalColor()) {
                    TransplantListItem(
                        ...
                        headlineContent = { Text("暂无本学期校园网用量数据") },
        ...
        is UiState.Error -> {
            CustomCard(color = cardNormalColor()) {
                TransplantListItem(
                    ...
                    supportingContent = { Text("可能未登录校园网自服务系统，或当前网络无法访问自服务接口") }
                )
            }
        }
...
private fun formatSchoolNetFlow(mb: Double): String {
    return if (mb >= 1024) {
