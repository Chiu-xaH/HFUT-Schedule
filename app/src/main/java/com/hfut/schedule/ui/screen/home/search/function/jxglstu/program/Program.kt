    ...
    when {
            ...
            if(showInfo) {
                courseInfo?.let {
                    planCoursesTransform(it)?.let { b ->
                        ProgramInfo(courseInfo = b,vm, hazeState, ifSaved){ showInfo = false }
                    }
                }
            }
        ...
        planCourses.isEmpty() -> {
            ...
            val total by produceState(initialValue = 0.0) {
                children.map { value += it.requireInfo?.requiredCredits ?: 0.0 }
            }

            LazyColumn {
                items(children.size, key = { it }) { item ->
                    val dataItem = children[item]
                    val credit = dataItem.requireInfo?.requiredCredits
                    val titles = dataItem.type?.nameZh + " | 学分要求 " + credit
                    AnimationCardListItem(
                        headlineContent = { Text(text = titles) },
                ...
                item { BottomTip(str = "合计 $total 学分") }
            }
        }
    }
}
