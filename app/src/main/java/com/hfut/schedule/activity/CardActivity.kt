...
@Composable
fun CardUI(vm : LoginSuccessViewModel,activity : Activity) {
    ...
    Scaffold(
        ...
        NavHost(navController = navController, startDestination = CardBarItems.BILLS.name, modifier = Modifier
            ...
            composable(CardBarItems.BILLS.name) { CardBills(vm,innerPadding) }
            composable(CardBarItems.COUNT.name) {  CardCounts(vm, innerPadding) }
            composable(CardBarItems.FUNCTION.name) { CardFunctions(vm,innerPadding) }
        }
