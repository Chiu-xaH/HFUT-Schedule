...
class CourseLiveUpdateReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                when (intent.action) {
                    CourseLiveUpdateScheduler.ACTION_SHOW,
                    CourseLiveUpdateScheduler.ACTION_REFRESH -> showCourseLiveUpdate(context, intent)
    ...
    private fun showCourseLiveUpdate(context: Context, intent: Intent) {
        ...
        AppNotificationManager.showCourseLiveUpdate(
            ...
            endMillis = endMillis,
            contentIntent = CourseLiveUpdateScheduler.buildOpenCourseIntent(context, courseName, startMillis)
        )
        CourseLiveUpdateScheduler.scheduleNextRefresh(
            context = context,
            courseName = courseName,
            place = intent.getStringExtra(CourseLiveUpdateScheduler.EXTRA_PLACE),
            teacher = intent.getStringExtra(CourseLiveUpdateScheduler.EXTRA_TEACHER),
