...
3. 所有页面管理归一化(Card、Shower、Xwx、Supabase 回归 SharedNav 管理) ，WebView、Fix 仍保持，WidgetConfig 删掉，Success 并入主逻辑，对 Starter 类进行重构  (基本完成,除 Success 其余已完成)
4. 模块整理：shared 模块改名 common-logic，common 模块改名 common-ui  (已完成)
5. 模块整理：network 模块移入 repo（解析层），并新建 RepoInf (基本完成，剩余少量函数需等步骤8完成才能继续)
...
7. 彻底去掉 Groovy Gradle (正在进行，剩余app模块和外部)
