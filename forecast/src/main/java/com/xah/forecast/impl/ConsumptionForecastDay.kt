package com.xah.forecast.impl

import com.xah.forecast.inf.ConsumptionForecastBase
...
class ConsumptionForecastDay(bean: BillBean) : ConsumptionForecastBase(bean) {
    ...
    override fun predict(): PredictResult {
        ...
        val daysSinceStart = dates.map { ChronoUnit.DAYS.between(base, it).toDouble() }

        // --- 1. 剔除偏离较大点（标准差法） ---
        val mean = amounts.average()
        val std = sqrt(amounts.sumOf { (it - mean) * (it - mean) } / amounts.size)

        val filteredIndices = amounts.mapIndexedNotNull { index, value ->
            if (abs(value - mean) <= 2 * std) index else null
        }
