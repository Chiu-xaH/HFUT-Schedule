...
internal class ConsumptionForecastMonth(bean: HuiXinBill) : ConsumptionForecastBase(bean) {
    ...
    override fun predict(): PredictResult {
        ...
        val amounts = months.map {
            map[it]!!
        }

        // 按月编号 x = 0, 1, 2...
        val x = months.indices.map {
            it.toDouble()
        }
        val y = amounts
