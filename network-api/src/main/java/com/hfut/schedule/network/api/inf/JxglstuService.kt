...
interface JxglstuService {
    ...
    //获取选课
    @FormUrlEncoded
    @Headers(Constant.PC_UA,"Content-Type: application/x-www-form-urlencoded")
    ...
    * */
    ...
    fun getLessonTimes(
        @Header("Cookie") cookie : String,
        @Body studentId : LessonTimeRequest
    ) : Call<ResponseBody>
    // 教材
    ...
    // 选课确认 随便传semesterId即可
    /*
     本学期漏选课程（必修） CURRENT_SEMESTER
     历史学期漏选课程（必修） HISTORY_SEMESTER
     未通过课程（必修） NAVER_NOT_PASSED
     */
    @FormUrlEncoded
    @Headers(Constant.PC_UA)
    @POST("for-std/course-select-confirm/info")
    fun getSelectCourseConfirmation(
        @Header("Cookie") cookie : String,
        @Field("studentId") studentId : Int,
        @Field("semesterId") semesterId : Int,
    ) : Call<ResponseBody>
}